...
export const genererRapportPeriodique = onSchedule('every monday 08:00', async (event) => {
  try {
    ...
    for (const doc of projetsSnapshot.docs) {
      ...
      if (depenses.length === 0 && rapports.length === 0) continue;
...
Mets en évidence que le chantier suit son cours.`;
      ...
      if (clientId) {
        await sendNotificationToUser(
          ...
          'Le point sur l\'avancement de votre chantier cette semaine est prêt.',
          'rapport',
          summaryRef.id
        );
      }
